...
public class FreeIpaConfigService {
    ...
    public FreeIpaConfigView createFreeIpaConfigs(Stack stack, Set<Node> hosts) {
        ...
        determineAndSetBackup(builder, stack);
    ...
    private void determineAndSetBackup(FreeIpaConfigView.Builder builder, Stack stack) {
        Backup backup = stack.getBackup();
        if (backup != null) {
            builder.withBackupEnabled(true)
                    .withMonthlyFullBackupEnabled(backup.isMonthlyFullEnabled())
                    .withHourlyBackupEnabled(backup.isHourlyEnabled())
                    .withInitialFullBackupEnabled(backup.isInitialFullEnabled())
                    .withBackupLocation(backup.getStorageLocation());
            if (backup.getS3() != null) {
                builder.withBackupPlatform(CloudPlatform.AWS.name());
                LOGGER.debug("Backups will be configured to use S3 output.");
            } else if (backup.getAdlsGen2() != null) {
                builder.withBackupPlatform(CloudPlatform.AZURE.name())
            ...
            builder.withBackupEnabled(false);
